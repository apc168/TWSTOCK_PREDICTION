...
def dataframe_to_html_table(df: pd.DataFrame, table_id: str = "data-table") -> str:
    ...
    for col in df.columns:
        ...
        thead += f'<th class="{cls}">{esc(col)}</th>'
    thead += "</tr></thead>"
...
def summary_cards(df: pd.DataFrame, file_name: str, truncated: bool, original_rows: int) -> str:
    ...
    cards.append(card("Rows", f"{original_rows:,}" + (" / truncated" if truncated else "")))
    cards.append(card("Columns", f"{len(df.columns):,}"))

    if "stock_id" in df.columns:
        cards.append(card("Stocks", f"{df['stock_id'].nunique():,}"))

    if "date" in df.columns:
        dates = pd.to_datetime(df["date"], errors="coerce")
        if dates.notna().any():
            cards.append(card("Date range", f"{dates.min().date()} ~ {dates.max().date()}"))

    if "prediction_5d" in df.columns:
        counts = df["prediction_5d"].value_counts(dropna=False).to_dict()
        cards.append(card("5D prediction", " / ".join(f"{k}:{v}" for k, v in counts.items())))

    if "prediction" in df.columns:
        counts = df["prediction"].value_counts(dropna=False).to_dict()
        cards.append(card("Prediction", " / ".join(f"{k}:{v}" for k, v in counts.items())))
