...
# -*- coding: utf-8 -*-
"""
Generate stocks.txt for Taiwan listed / OTC common stocks.

預設：只產生上市、上櫃「普通股票」，不包含 ETF。

Usage:
    python make_stocks_txt.py

Outputs:
    stocks.txt          # one stock id per line, for example.py --stock-file
    stocks_meta.csv     # metadata for review/debugging

如果之後又想包含 ETF，可加：
    python make_stocks_txt.py --include-etf
"""
...
def is_common_stock_code(code: str) -> bool:
    # Keep normal listed/OTC common stocks.
    # This intentionally excludes ETFs, preferred shares, rights, warrants,
    # special securities with letters, etc.
...
def write_outputs(records: list[dict[str, str]], output_path: Path, meta_path: Path) -> None:
    if not records:
        raise RuntimeError("No stock ids were collected. Please check network access or TWSE ISIN page format.")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate TWSE/TPEx stocks.txt. Default: common stocks only, no ETFs.")
    ...
    parser.add_argument("--include-etf", action="store_true", help="Also include ETFs. Default is stock-only.")
